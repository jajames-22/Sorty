...
class EditProfileActivity : AppCompatActivity() {
    ...
    private fun loadUserData() {
        val loggedInEmail = sessionManager.getEmail()
        if (loggedInEmail != null) {
            // Fetch ONLY the user belonging to this session
            val user = db.getUserByEmail(loggedInEmail)
            if (user != null) {
                originalUser = user
                ...
                if (!user.imageUri.isNullOrEmpty()) {
                    try {
                        ...
                        binding.ivProfilePreview.setImageResource(R.drawable.ic_file_empty)
                    }
                }
            }
        } else {
    ...
    private fun validateInputsAndConfirm() {
        ...
        // Branching: Show warning for Email change, standard confirm for others
        if (emailChanged) {
    ...
    private fun showEmailWarningDialog() {
        ...
        tvTitle.text = "Change Email?"
        tvMessage.text = "Changing your email will disconnect your account from your current shared folders and notes. You might lose access to all your data!"
        btnConfirm.text = "I Understand"
